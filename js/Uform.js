...
function generateUform(W, L, H) {
    ...
    function createScene(){
        ...
        // create a camera
        const camera = new BABYLON.ArcRotateCamera('camera', -Math.PI/2, Math.PI/2, 30, new BABYLON.Vector3(0, 2.5, -5), scene);
        ...
        // create a light
        const light = new BABYLON.HemisphericLight('light', new BABYLON.Vector3(0, H, 0), scene);
        ...
        var buildFromPlan = function(ground, walls, ply, H, options, scene) {
            ...
            for(var w = 0; w < currentLength/3; w++) { //currentlength*3 = 8 corners in and out 
            ...
            // (x, z) = (15, 10)
            for(var p = 0; p < positions.length / 12; p++) { 
                /*uvs.push(-(positions[3 * p + 2] - (-W/2)) /L, (positions[3 * p + 1] - (W/2)) /L);
                uvs.push((positions[3 * p + 2] - (-W/2)) /L, (positions[3 * p + 1] - (W/2)) /L);
                uvs.push((positions[3 * p + 0] - (-W/2)) /L, -(positions[3 * p + 2] - (W/2)) /L);
                uvs.push((positions[3 * p + 0] - (-W/2)) /L, (positions[3 * p + 2] - (W/2)) /L); */
                
                uvs.push((positions[3 * p + 0] - (-W/2)) /L, -(positions[3 * p + 1] - (W/2)) /L);
            ...
            var texture1 = new BABYLON.Texture("../assets/tiles.jpg", scene);
            // texture.uScale = 10;
            // texture.vScale = 10;
        ...
        var windowSpace1 = new windowSpace(window1, 1.5, 0.4);
        walls[0].windowSpaces = [windowSpace1];*/
        
        var ply = 0;
        // var height = 5;
        var ground = BABYLON.MeshBuilder.CreateGround("ground", {height: L, width: W}, scene);
        var mat0 = new BABYLON.StandardMaterial("mat0", scene);
        //mat0.diffuseColor = new BABYLON.Color3(1, 1, 0);
        var texture0 = new BABYLON.Texture("../assets/tiles.jpg", scene);
        //texture0.uScale = 10;
        // texture0.vScale = 10;
